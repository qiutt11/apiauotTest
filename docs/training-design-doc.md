...
YAML / JSON / Excel 用例文件
        │
        ▼
   load_testcases() / load_yaml_datasets()
        │
        ▼
...
| `load_yaml_datasets(file_path)` | YAML 数据驱动文件加载（每项=一组嵌套测试数据） |
| `scan_testcase_files(directory)` | 扫描目录下所有支持的文件 |

**`load_yaml_datasets` 说明**：加载 YAML 列表文件，每项是一组嵌套 dict。YAML 原生支持嵌套，无需 JSON 解析。
...
TestCaseFile.collect()
  │
  ├── case 有 yaml_source + steps?
  │     │
  │     ├── YES → load_yaml_datasets(data_path)
  │     │          │
  │     │          ├── dataset 0 → DataDrivenItem("用例名[张三]")
  │     │          ├── dataset 1 → DataDrivenItem("用例名[李四]")
  │     │          └── ...
  │     │
  │     └── NO → TestCaseItem（走原有逻辑）
```

### 4.2 DataDrivenItem.runtest() 执行流程

```
1. pool.clear_module()（文件切换时）
2. for step in steps:
   a. copy.deepcopy(step) → step_case
   b. body_from_yaml? → dataset 去掉 label → step_case["body"]
   c. validate_from_yaml? → _build_yaml_validations(dataset, mapping) → 追加到 validate
...
| 函数 | 职责 |
|------|------|
| `_get_by_path(data, path)` | 按点分路径从嵌套 dict 取值 |
| `_build_yaml_validations(dataset, mapping)` | 按路径映射生成 eq 断言（支持 `[]` 数组通配） |
| `_expand_array_mapping(...)` | 展开数组通配符，按下标生成每个元素的断言 |
| `_flatten_value(jsonpath, value, validations)` | 递归展开嵌套 dict/list 为 eq 断言 |

### 4.4 路径映射工作原理

```
mapping: {"userInfo.contacts[].type": "$.data.contactList[].contactType"}
dataset: {"userInfo": {"contacts": [{"type": "phone"}, {"type": "email"}]}}

→ 处理过程：
  1. 按 [] 拆分 data_path: before="userInfo.contacts", after=".type"
  2. _get_by_path(dataset, "userInfo.contacts") → [{"type":"phone"}, {"type":"email"}]
  3. 遍历数组:
     i=0: _get_by_path(item, "type") → "phone" → eq: [$.data.contactList[0].contactType, "phone"]
     i=1: _get_by_path(item, "type") → "email" → eq: [$.data.contactList[1].contactType, "email"]
...
pytest 启动
  ...
  │   ├── --level 过滤
  │   ├── 普通用例 → TestCaseItem
  │   └── YAML 数据驱动 → DataDrivenItem × N 组
  │
  ├── TestCaseItem.runtest() / DataDrivenItem.runtest()
  │   └── run_testcase() → 核心执行
  │
...
TestCaseFile.collect()
  ...
  └── for case in testcases:
      ...
      │   ├── load_yaml_datasets() → datasets
      │   └── for dataset in datasets:
      │       └── yield DataDrivenItem(name="用例名[label]")
      │
...
| test_yaml_driven.py | 21 | load_yaml_datasets、路径取值、映射断言生成、递归展开 |
...
| `data_loader.py` | 140 | YAML/JSON/Excel 数据加载 + load_yaml_datasets |
...
| 文件 | 职责 |
|------|------|
| `conftest.py` | 钩子注册、框架初始化、用例收集（TestCaseItem + DataDrivenItem）、统计 |
