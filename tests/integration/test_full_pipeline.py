...
# ---------------------------------------------------------------------------
# Test 10: Excel-driven pipeline (save → detail with validation from Excel)
# ---------------------------------------------------------------------------
class TestExcelDrivenPipeline:
    """Excel 驱动用例的完整流水线测试：保存 → 提取 ID → 查看详情 → 用 Excel 数据断言。"""

    def test_save_and_verify_detail(self, tmp_path):
        """Excel 两行数据 → 每行执行 save + detail，验证变量传递和断言生成。"""
        import responses
        ...
        try:
            # Mock: 保存接口，返回 id
            responses.add(
                responses.POST, "http://mock-api.local/api/user/save",
                json={"code": 0, "data": {"id": 1001}},
                status=200,
            )
            # Mock: 详情接口，返回完整数据（第一行数据 张三）
            responses.add(
                responses.GET, "http://mock-api.local/api/user/detail/1001",
                json={"code": 0, "data": {"name": "张三", "age": 25, "phone": "138xxx"}},
                status=200,
            )
            # Mock: 第二行的 save
            responses.add(
                responses.POST, "http://mock-api.local/api/user/save",
                json={"code": 0, "data": {"id": 1002}},
                status=200,
            )
            # Mock: 第二行的 detail
            responses.add(
                responses.GET, "http://mock-api.local/api/user/detail/1002",
                json={"code": 0, "data": {"name": "李四", "age": 30, "phone": "139xxx"}},
                status=200,
            )
            ...
            ws.append(["张三", 25, "138xxx"])
            ws.append(["李四", 30, "139xxx"])
            ...
            for row_idx, row_data in enumerate(rows):
                ...
                save_case = {
                    ...
                    "method": "POST",
                    "url": "/api/user/save",
                    "body": _build_body_from_excel(row_data, True),
                    "extract": {"id": "$.data.id"},
                    "validate": [{"eq": ["$.code", 0]}],
                }
                r1 = run_testcase(save_case, cfg["base_url"], pool, 10, hooks,
                                  global_headers=cfg.get("global_headers", {}))
                assert r1["passed"] is True
                assert pool.get("id") is not None

                # Step 2: 查看详情（用 Excel 数据生成断言）
            ...
            # 验证第一行 save 的 body 内容
            first_save_body = json.loads(responses.calls[0].request.body)
            assert first_save_body["name"] == "张三"
            assert first_save_body["age"] == 25

            # 验证第二行 save 的 body 内容
            second_save_body = json.loads(responses.calls[2].request.body)
            assert second_save_body["name"] == "李四"
            assert second_save_body["age"] == 30
        finally:
            responses.stop()
            responses.reset()

    def test_field_mapping_in_save_and_validate(self, tmp_path):
        """field_mapping 场景：Excel 列名与接口字段名不一致时的映射。"""
        import responses
        ...
        try:
            responses.add(
                responses.POST, "http://mock-api.local/api/user/save",
                json={"code": 0, "data": {"id": 2001}},
                status=200,
            )
            responses.add(
                responses.GET, "http://mock-api.local/api/user/detail/2001",
                json={"code": 0, "data": {"user_name": "张三", "phone_number": "138xxx"}},
                status=200,
            )
            ...
            # 模拟 Excel 行数据
            row_data = {"name": "张三", "phone": "138xxx"}

            # Save with body field_mapping
            body = _build_body_from_excel(
                row_data,
                {"field_mapping": {"name": "userName", "phone": "phoneNumber"}},
            )
            save_case = {
                "name": "保存",
                "method": "POST",
            ...
            # 验证 body 使用了映射后的字段名
            sent_body = json.loads(responses.calls[0].request.body)
            assert sent_body["userName"] == "张三"
            assert sent_body["phoneNumber"] == "138xxx"

            # Detail with validate field_mapping
            detail_validations = _build_excel_validations(
                row_data,
                {
                    "prefix": "$.data",
    ...
    def test_detail_validation_failure(self, tmp_path):
        """详情返回值与 Excel 不匹配时，断言应失败。"""
        import responses

        responses.start()
        try:
            responses.add(
                responses.GET, "http://mock-api.local/api/user/detail/1",
                json={"code": 0, "data": {"name": "错误的名字", "age": 99}},
                status=200,
            )
            ...
            detail_case = {
                "name": "详情校验应失败",
                "method": "GET",
                "url": "/api/user/detail/1",
                "validate": detail_validations,
            }
    ...
    def test_complex_nested_validation(self, tmp_path):
        """复杂嵌套值（数组 + 对象）的递归断言。"""
        import responses
        ...
        try:
            ...
            # 应该生成 5 条断言：name + tags[0] + tags[1] + address.city + address.street
            assert len(detail_validations) == 5
            ...
            assert r["passed"] is True
            # 所有断言（1 + 5 = 6）都应通过
            assert len(r["validations"]) == 6
            assert all(v["passed"] for v in r["validations"])
        finally:
            responses.stop()
            responses.reset()
...
# ---------------------------------------------------------------------------
# Test 11: Subprocess conftest pipeline (real pytest collection → execution → stats)
# ---------------------------------------------------------------------------
