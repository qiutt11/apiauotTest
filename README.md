...
- **接口依赖** — 同文件 `extract` + `${变量名}` 传递，跨文件 `depends` 自动执行依赖并共享变量
...
- **多渠道通知** — 邮件 + 飞书机器人，失败自动告警并 @相关人，通知卡片支持"查看报告"按钮
- **飞书机器人触发** — 飞书群发 `/run` 指令即可远程触发测试，自动推送结果和报告链接
- **在线报告** — 内置报告托管服务，其他电脑通过浏览器直接查看 HTML 报告
...
├── common/          # 框架核心代码（无需修改）
├── tests/           # 框架测试（175个：149单元+21集成+5过滤）
├── reports/         # 报告输出（自动生成）
├── logs/            # 日志输出（自动生成）
├── run.py           # 运行入口
├── conftest.py      # pytest 配置
├── scripts/         # 工具脚本（飞书机器人服务等）
