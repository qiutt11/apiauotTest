...
- **数据驱动** — 用例写在 YAML/JSON/Excel 文件中，新增用例零代码，支持 Excel 驱动的保存+详情验证模式
...
├── common/          # 框架核心代码（无需修改）
├── tests/           # 框架测试（158个：132单元+21集成+5过滤）
├── reports/         # 报告输出（自动生成）
├── logs/            # 日志输出（自动生成）
├── run.py           # 运行入口
...
| [docs/excel-driven-guide.md](docs/excel-driven-guide.md) | Excel 驱动用例使用手册（保存+详情验证、跨系统调用） |
...
| [docs/excel-driven-design.md](docs/excel-driven-design.md) | Excel 驱动用例设计文档（架构、模块设计、测试覆盖） |
